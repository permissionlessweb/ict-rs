...
use crate::quickspawn::snapshot::{ChainSnapshot, DeployedContract, SpawnedChainSet, SpawnedNode, ValidatorEntry};
...
/// A spawned chain node with connection information.
///
/// Returned by [`QuickSpawnManager::spawn`]. The caller uses
/// `host_rpc_port` and `host_grpc_port` to connect a cw-orch Daemon.
#[deprecated(since = "0.2.0", note = "Use SpawnedChainSet instead for multi-node support")]
pub struct SpawnedChain {
    /// The ChainNode wrapping the running container.
    pub node: ChainNode,
    /// Host-mapped RPC port (for tendermint-rs / cw-orch Daemon).
    pub host_rpc_port: u16,
    /// Host-mapped gRPC port (for cw-orch Daemon).
...
impl SpawnedChain {
    /// RPC URL for cw-orch Daemon config.
    pub fn rpc_url(&self) -> String {
        format!("http://127.0.0.1:{}", self.host_rpc_port)
    }

    /// gRPC URL for cw-orch Daemon config.
    pub fn grpc_url(&self) -> String {
        format!("http://127.0.0.1:{}", self.host_grpc_port)
    }
}

// Best-effort: stop+remove container on drop so panicking tests don't leak.
// If stop() was already called, the runtime returns a harmless error
// (container not found) which is swallowed.
impl Drop for SpawnedChain {
    fn drop(&mut self) {
        match tokio::runtime::Handle::try_current() {
            Ok(handle) => {
                let container_id = self.node.container_id.clone();
                let runtime = self.node.runtime.clone();
            ...
            Err(_) => warn!(
                "SpawnedChain dropped outside tokio runtime — container may leak. \
                 Call stop() explicitly in CI."
            ),
        }
    }
}
...
impl<S: SnapshotStore> QuickSpawnManager<S> {
        ...
        let node_id = node.node_id().await
            .map_err(|e| IctError::Chain {
                ...
                source: anyhow::anyhow!(
                    "failed to get node_id for validator {index}: {e}"
                ),
            })?;
    ...
    pub async fn bootstrap(
        ...
        // ── 6. Store snapshot ───────────────────────────────────────
        let snapshot_id = self.store.save(&snapshot).await
            .map_err(|e| IctError::Chain {
                chain_id: chain_id.clone(),
                source: e.into(),
            })?;
    ...
    pub async fn spawn(
        ...
        let snapshot = self.store.load(snapshot_id).await
            .map_err(|e| IctError::Chain {
        ...
        let runtime: Arc<dyn RuntimeBackend> = crate::runtime::IctRuntime::Docker(Default::default())
            .into_backend()
            .await?;

        let network_name = format!("qs-spawn-{}-{}",
        ...
        let init_cmd: &[&str] = match chain_cfg.genesis_style {
            GenesisStyle::Modern => &[
                &chain_cfg.bin, "genesis", "init", &moniker,
                "--home", &spawn_node.node.home_dir, "--chain-id", &chain_cfg.chain_id,
            ],
            GenesisStyle::Legacy => &[
                &chain_cfg.bin, "init", &moniker,
                "--home", &spawn_node.node.home_dir, "--chain-id", &chain_cfg.chain_id,
            ],
        };
        let init_output = spawn_node.node.exec_raw(init_cmd, &[]).await
            .map_err(|e| IctError::Chain {
                chain_id: chain_cfg.chain_id.clone(),
        ...
        if init_output.exit_code != 0 {
            ...
            return Err(IctError::Chain {
                ...
                source: anyhow::anyhow!(
                    ...
                    spawn_node.node.hostname, init_output.exit_code, stderr.trim()
                ),
            });
        }
        ...
        write_node_file(
            ...
            &serde_json::to_string_pretty(&snapshot.genesis_json)
                .map_err(|e| IctError::Chain {
                    chain_id: chain_cfg.chain_id.clone(),
                    source: e.into(),
                })?,
        ...
        if !primary_entry.validator_key_json.is_empty() {
            write_node_file(
                &spawn_node.node,
                &format!("{}/config/priv_validator_key.json", spawn_node.node.home_dir),
    ...
    pub async fn spawn_multi(
        ...
        let test_name = format!("quickspawn-spawn-{}", snapshot_id.get(..16).unwrap_or("deadbeef"));
        ...
        for (_global_idx, is_validator, entry) in &node_descriptors {
            ...
            match node {
                ...
                Err(e) => {
                    ...
                    let all_created: Vec<&SpawnedNode> = spawned_validators.iter()
        ...
        let all_nodes: Vec<&SpawnedNode> = spawned_validators.iter()
            .chain(spawned_full_nodes.iter())
            .collect();

        for spawn_node in &all_nodes {
            let moniker = format!("qs-{}-{}",
                if spawn_node.node.is_validator { "val" } else { "fn" },
        ...
        let genesis_content = serde_json::to_string_pretty(&snapshot.genesis_json)
            .map_err(|e| IctError::Chain {
                chain_id: chain_cfg.chain_id.clone(),
                source: e.into(),
            })?;
        ...
        for (_global_idx, is_validator, entry) in &node_descriptors {
            ...
            let spawn_node = if *is_validator {
                spawned_validators.iter()
                    .find(|n| n.node.index == spawn_node_idx)
            } else {
                spawned_full_nodes.iter()
                    .find(|n| n.node.index == spawn_node_idx)
        ...
        for spawn_node in &all_nodes {
            ...
            let entry_id = if spawn_node.node.is_validator {
                snapshot.validators.iter()
                    .find(|v| v.index == spawn_node.node.index)
                    .and_then(|v| v.node_id.as_deref())
            } else {
                snapshot.full_nodes.iter()
                    .find(|v| v.index == spawn_node.node.index)
                    .and_then(|v| v.node_id.as_deref())
            }.ok_or_else(|| IctError::Chain {
                chain_id: chain_cfg.chain_id.clone(),
                source: anyhow::anyhow!(
                    "missing node_id for {} (snapshot did not capture it)", spawn_node.node.hostname
                ),
            })?;
    ...
    pub async fn stop(&self, spawned: &SpawnedChain) -> Result<()> {
        if let Some(ref id) = spawned.node.container_id {
            ...
            let _ = spawned.node.runtime.stop_container(id).await
    ...
    pub async fn stop_set(&self, set: &SpawnedChainSet) -> Result<()> {
        let all_nodes: Vec<&SpawnedNode> = set.validators.iter()
            .chain(set.full_nodes.iter())
            .collect();

        for spawn_node in &all_nodes {
            if let Some(ref id) = spawn_node.node.container_id {
                let _ = spawn_node.node.runtime.stop_container(id).await
...
async fn write_node_file(node: &ChainNode, path: &str, content: &str) -> Result<()> {
    ...
    node.exec_raw(
        &["sh", "-c", "echo \"$1\" | base64 -d > \"$2\"", "--", &b64, path],
        &[],
    ).await?;
    Ok(())
}
...
async fn create_spawned_node(
    ...
    let opts = ContainerOptions {
        ...
        entrypoint: None,
        ports: vec![
            PortBinding { host_port: 0, container_port: 26657, protocol: "tcp".to_string() },
            PortBinding { host_port: 0, container_port: 9090, protocol: "tcp".to_string() },
            PortBinding { host_port: 0, container_port: 26656, protocol: "tcp".to_string() },
        ],
        volumes: vec![],
        labels: vec![
            ("ict-rs.quickspawn".to_string(), "true".to_string()),
        ],
    ...
    let host_rpc_port = runtime.get_host_port(&container_id, 26657, "tcp")
        .await?
        .ok_or_else(|| IctError::Chain {
            chain_id: chain_cfg.chain_id.clone(),
            source: anyhow::anyhow!("failed to resolve RPC port for {}", node.hostname),
        })?;
    let host_grpc_port = runtime.get_host_port(&container_id, 9090, "tcp")
        .await?
        .ok_or_else(|| IctError::Chain {
            chain_id: chain_cfg.chain_id.clone(),
            source: anyhow::anyhow!("failed to resolve gRPC port for {}", node.hostname),
        })?;
    let host_p2p_port = runtime.get_host_port(&container_id, 26656, "tcp")
        .await?
        .ok_or_else(|| IctError::Chain {
            chain_id: chain_cfg.chain_id.clone(),
...
async fn configure_node_config(
    ...
    // RPC bind address: scope to [rpc] section
